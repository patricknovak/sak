      ...
      <div className="scroll-x flex shrink-0 gap-1 border-b border-line px-2 py-2 lg:card lg:w-56 lg:flex-col lg:overflow-y-auto lg:p-2">
        {channels.map((x) => (
          <button key={x.c} onClick={() => setParams({ c: x.c })}
            className={`relative flex shrink-0 items-center gap-1.5 rounded-full px-3 py-1.5 text-sm font-semibold lg:rounded-xl ${channel === x.c ? 'bg-white text-ice' : 'bg-boards text-slate-300'}`}>
